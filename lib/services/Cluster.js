...
Cluster.read = function(){
  ...
  switch(queryMode){
    case 'all':
      ...
      break;
    case 'byId':
      results = clusters.filter(queryByIdFilter.bind(null, id))[0] || null;
      break;
    case 'byName':
      results = clusters.filter(queryByNameFilter.bind(null, query.name))[0] || null;
      break;
  }
...
Cluster.update = function(id, data, cb){
  Cluster.read(id, null, errHandler( cb, function(results){
    var err = null;
